...
class InversePipelineXLPartial(StableDiffusionXLPipeline):
    @torch.no_grad()
    def __call__(
        self,
        ...
        **kwargs,
        ...
        if image is not None:
            ...
            with torch.no_grad():
                x0_dec = self.vae.decode(x0_enc.detach() / self.vae.config.scaling_factor, return_dict=False)[0]
                image_x0_dec = self.image_processor.postprocess(x0_dec, output_type="pil")
        else:
            image_x0_dec = [None]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if threshold_timestep and t > threshold_timestep:
                    break
        ...
        if not output_type == "latent":
            ...
            # latents = torch.load("outputs/DDIM_inversion_normal_horse.pt").to(self.vae.device)
        ...
        return StableDiffusionXLPipelineOutput(images=image, noise=latents, decode_images=image_x0_dec, intermediate_latents=intermediate_latents, inversed_intermediate_latents=inversed_intermediate_latents, nsfw_content_detected=has_nsfw_concept)
