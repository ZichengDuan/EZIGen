...
@dataclass
class StableDiffusionPipelineOutput_main_v2(BaseOutput):
    """
...
class StableDiffusionPipeline_main(
    DiffusionPipeline,
    ...
    def __init__(
        ...
        unet: UNet2DConditionModel_main,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if latents is not None:
            timesteps = timesteps[latents_steps:]
        else:
            latents_steps = 0
        ...
        if args.do_editing and foreground_mask is not None and args.skip_adapter_ratio != 1: 
            ...
            background_mask = torch.ones_like(latents, dtype=latents.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    latent_model_input, # -399
                    t,
                    encoder_hidden_states=prompt_embeds, # tensor(-26480.6406, device='cuda:0')
                ...
                if self.do_classifier_free_guidance:
                    ...
                    # noise_pred = uncon_noise_pred + self.guidance_scale * (cond_noise_pred - uncon_noise_pred)

                if self.do_classifier_free_guidance and self.guidance_rescale > 0.0:
                    noise_pred = rescale_noise_cfg(noise_pred, noise_pred_text, guidance_rescale=self.guidance_rescale)

                # cond_latent_model_input = latents.clone()
                # cond_latent_model_input = self.scheduler.scale_model_input(cond_latent_model_input, t)

                # uncon_latent_model_input = latents.clone()
                # uncon_latent_model_input = self.scheduler.scale_model_input(uncon_latent_model_input, t)

                # # conditional
                # cond_noise_pred, _ = self.unet(
                ...
                # uncon_noise_pred, _ = self.unet(
                #     uncon_latent_model_input,
                #     t,
                #     encoder_hidden_states=prompt_embeds[:1, :, :],
                #     timestep_cond=timestep_cond,
                #     cross_attention_kwargs=self.cross_attention_kwargs,
                ...
                # if self.do_classifier_free_guidance and self.guidance_rescale > 0.0:
                #     noise_pred = rescale_noise_cfg(noise_pred, cond_noise_pred, guidance_rescale=self.guidance_rescale)


                # compute the previous noisy sample x_t -> x_t-1
                latents = self.scheduler.step(noise_pred, t, latents, **extra_step_kwargs, return_dict=False)[0]

                # if args.use_foreground_mask and foreground_mask is not None and not args.initial_loop and args.skip_adapter_ratio != 1:
                #     latents = latents * foreground_mask + original_latents * background_mask
                
                if inversed_intermediate_latents is not None and foreground_mask is not None and args.skip_adapter_ratio != 1:
                    latents =  latents * foreground_mask + inversed_intermediate_latents[i] * background_mask
                ...
                # # decode each noisy image
                # if args.skip_adapter_ratio == 0 or args.initial_loop:
                #     noisy_image = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False, generator=generator)[0]
                #     noisy_image = self.image_processor.postprocess(noisy_image, output_type=output_type, do_denormalize=[True] * noisy_image.shape[0])
                #     noisy_image[0].save(f"results/noisy_outputs/noisy_image_{args.skip_adapter_ratio}_{i}.png")
