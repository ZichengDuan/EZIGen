...
def add_noise_to_image(noise_step, generator, args, img: PIL.Image, vae, train_transforms, noise=None, noise_scheduler=None):
    """
...
def extract_subject_features(args, image_paths, reference_unet, text_encoder, tokenizer, vae, noise_scheduler, subject_noise, weight_dtype, transforms, text="", device="cuda:0", subject_denoise_timestep = None, generator=None):
    ...
    inputs = inputs[:, None, :]
    encoder_hidden_states_ref = text_encoder(inputs.to(reference_unet.device), return_dict=False)[0]
    ...
    # self_attn_results_all_blocks: [Block1 features for 10 ref img, Block2, ...,Block16]
    _, self_attn_results_all_blocks = reference_unet(noisy_comp_latents, subject_denoise_timestep, encoder_hidden_states_ref, return_dict=False, args=args)
    self_attn_results_all_blocks = [block_feat.reshape(1, -1, block_feat.shape[-1]) if block_feat is not None else None for block_feat in self_attn_results_all_blocks] # B, ref_image_patches, dim 
    
    
    return self_attn_results_all_blocks
