...
from models.reference_unet.transformer_2d_ref import Transformer2DModel_ref
...
def get_down_block(
    ...
    if down_block_type == "DownBlock2D":
        ...
        return CrossAttnDownBlock2D_ref(
            num_layers=num_layers,
...
def get_mid_block(
    ...
    if mid_block_type == "UNetMidBlock2DCrossAttn":
        return UNetMidBlock2DCrossAttn_ref(
            transformer_layers_per_block=transformer_layers_per_block,
            in_channels=in_channels,
            temb_channels=temb_channels,
            dropout=dropout,
...
class UNetMidBlock2DCrossAttn_ref(nn.Module):
    def __init__(
        self,
        in_channels: int,
        temb_channels: int,
    ...
    def forward(
        ...
        for i, (attn, resnet) in enumerate(zip(self.attentions, self.resnets[1:])):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states, self_attn_res = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
                    cross_attention_kwargs=cross_attention_kwargs,
                    attention_mask=attention_mask,
...
class CrossAttnDownBlock2D_ref(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    def forward(
        ...
        return hidden_states, output_states, self_attn_results
...
class CrossAttnUpBlock2D_ref(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
