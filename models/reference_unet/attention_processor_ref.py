...
class Attention_ref(nn.Module):
    ...
    Parameters:
        ...
        processor (`AttnProcessor`, *optional*, defaults to `None`):
            The attention processor to use. If `None`, defaults to `AttnProcessor2_0_ref` if `torch 2.x` is used and
    ...
    def __init__(
        ...
        # We use the AttnProcessor2_0_ref by default when torch 2.x is used which uses
        # torch.nn.functional.scaled_dot_product_attention for native Flash/memory_efficient_attention
        # but only if it has the default `scale` argument. TODO remove scale_qk check when we move to torch 2.1
        if processor is None:
            processor = (
                AttnProcessor2_0_ref() if hasattr(F, "scaled_dot_product_attention") and self.scale_qk else AttnProcessor()
            )
    ...
    def get_processor(self, return_deprecated_lora: bool = False) -> "AttentionProcessor":
        ...
        # serialization format for LoRA Attention_ref Processors. It should be deleted once the integration
    ...
    def forward(
        ...
        **cross_attention_kwargs,
        ...
        r"""
        The forward method of the `Attention_ref` class.
        ...
        """
        # The `Attention_ref` class can call different attention processors / attention functions
    ...
    def batch_to_head_dim(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        is the number of heads initialized while constructing the `Attention_ref` class.
    ...
    def head_to_batch_dim(self, tensor: torch.Tensor, out_dim: int = 3) -> torch.Tensor:
        ...
        the number of heads initialized while constructing the `Attention_ref` class.
...
class XFormersAttnAddedKVProcessor:
    ...
    Args:
        attention_op (`Callable`, *optional*, defaults to `None`):
            The base
            [operator](https://facebookresearch.github.io/xformers/references/ops.html#xformers.ops.AttentionOpBase) to
...
class AttnProcessor2_0_ref:
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("AttnProcessor2_0_ref requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
...
class CustomDiffusionXFormersAttnProcessor(nn.Module):
    ...
    attention_op (`Callable`, *optional*, defaults to `None`):
        The base
        [operator](https://facebookresearch.github.io/xformers/references/ops.html#xformers.ops.AttentionOpBase) to use
...
class LoRAAttnProcessor(nn.Module):
    ...
    def __call__(self, attn: Attention_ref, hidden_states: torch.FloatTensor, *args, **kwargs) -> torch.FloatTensor:
...
class LoRAAttnProcessor2_0(nn.Module):
    ...
    def __call__(self, attn: Attention_ref, hidden_states: torch.FloatTensor, *args, **kwargs) -> torch.FloatTensor:
        ...
        attn._modules.pop("processor")
        attn.processor = AttnProcessor2_0_ref()
        return attn.processor(attn, hidden_states, *args, **kwargs)
...
class IPAdapterAttnProcessor(nn.Module):
    r"""
    Attention_ref processor for Multiple IP-Adapater.
...
class IPAdapterAttnProcessor2_0(torch.nn.Module):
    r"""
    Attention_ref processor for IP-Adapater for PyTorch 2.0.
