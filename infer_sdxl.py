...
def loop_infer(args, subject_img_paths, subject_features, vae, noise_scheduler, weight_dtype, target_prompt, subject_prompts, train_transforms, generator, init_image, sim_threshold=0.98, pipeline=None, output_root=None, post_fix=None, reference_unet=None, exclip=None, inverse_pipeline=None, main_unet=None, clip_model=None, clip_processor=None, foreground_mask=None, initial_image_size = None, source_image_path=None):
    """
    input: 
    """
    ...
    while ((cur_loop_num < max_num_loop and sim < sim_threshold) or cur_loop_num < min_num_loop):
        if args.do_editing:
            ...
            valid_steps = min(noise_scheduler.timesteps, key=lambda x: abs(x - noise_step))  # 找到最近的值
            noisy_latents = add_noise_to_image(noise_step = valid_steps, args=args, img=loop_image, vae=vae, train_transforms=train_transforms, noise_scheduler=noise_scheduler)

        denoise_step = valid_steps
...
def iteration_wrapper(args, accelerator, subject_img_paths, main_unet, reference_unet, text_encoder_one, text_encoder_two, tokenizer_one, tokenizer_two, vae,  noise_scheduler, weight_dtype, target_prompt, subject_prompts, train_transforms, generator=None, output_root=None, post_fix="", pipeline=None, exclip=None, inverse_pipeline=None, clip_model=None, clip_processor=None, source_image_path=None, foreground_mask_path=None):
    ...
    subject_features  = extract_subject_features_sdxl(args, subject_img_paths, reference_unet, text_encoder_one, text_encoder_two, tokenizer_one, tokenizer_two, vae,  noise_scheduler, None,  weight_dtype, train_transforms, text=subject_prompts, subject_denoise_timestep=args.subject_denoise_timestep, device=reference_unet.device, generator=generator.manual_seed(int(args.seed)))
