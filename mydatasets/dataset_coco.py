...
class DatasetCOCO_sdxl(Dataset):
    ...
    def load_data(self):
        ...
        if 'train' in self.train_split:
            print("== COCO2014, using training set! ==")
            ...
            total_images = len(train_coco_cap.imgs)
            for key in tqdm(train_coco_cap.imgs.keys(), total=total_images, desc='Processing images'):
        ...
        if 'val' in self.train_split:
            print("== COCO2014, using validation set! ==")
            ...
            total_images = len(val_coco_cap.imgs)
            for key in tqdm(val_coco_cap.imgs.keys(), total=total_images, desc=f'Processing images'):
...
class DatasetCOCO_sd21(Dataset):
...
if __name__ == "__main__":
    ...
    coco_data = DatasetCOCO_sdxl("data/coco2014", transform=transform, tokenizer=tokenizer)
    # coco_data.load_data()
